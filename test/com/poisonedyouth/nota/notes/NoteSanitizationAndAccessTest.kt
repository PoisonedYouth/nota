...
@ActiveProfiles("test")
@Transactional
class NoteSanitizationAndAccessTest @Autowired constructor(
    ...
    fun `findAllNotes excludes archived and findAllArchivedNotes returns only archived`() {
        ...
        val n2 = noteRepository.save(Note(title = "b", content = "c", user = alice, createdAt = now, updatedAt = now, archived = true, archivedAt = now))
